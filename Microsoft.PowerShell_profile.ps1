...
function ls {
    ...
    # Обрезка с учётом ANSI, добавляет '../' (для папок)
    function Truncate-WithAnsi-Dir($s, $maxLen) {
        $plain = $s -replace "\[[\d;]*m", ''
        if ($plain.Length -le $maxLen) { return $s }
        $maxPlainLen = $maxLen - 3
        ...
        foreach ($c in $s.ToCharArray()) {
            ...
            $res += $c
            if ($ansi -and $c -eq "m") { $ansi = $false }
        }
        return $res + '..[0m/'
    }

    function Truncate-WithAnsi-File($s, $maxLen) {
        $plain = $s -replace "\[[\d;]*m", ''
        if ($plain.Length -le $maxLen) { return $s }
        $maxPlainLen = $maxLen - 3
        $res = ''; $plainCount = 0; $ansi = $false
    ...
    for ($i=0; $i -lt $rows; $i++) {
        ...
        for ($j=0; $j -lt $cols; $j++) {
            ...
            if ($idx -lt $items.Count) {
                ...
                if ($item.PSIsContainer) {
                    $nameRaw = $item.Name + '[0m/'
                    $nameColored = "[96m$nameRaw[0m"
                    $pad = $colWidth - (Get-PlainLength $nameColored)
                    if ($pad -gt 0) {
                        $nameColored += ' ' * $pad
                    } else {
                        $nameColored = Truncate-WithAnsi-Dir $nameColored $colWidth
                    }
                } else {
                    $nameRaw = $item.Name
                    $nameColored = $nameRaw
                    ...
                    if ($pad -gt 0) {
                        ...
                        $nameColored = Truncate-WithAnsi-File $nameColored $colWidth
                    }
                }
                $line += $nameColored
            }
        ...
        if ((Get-PlainLength $line) -gt $maxWidth) {
            $line = Truncate-WithAnsi-File $line $maxWidth
        }w
        Write-Output $line
    }
}
